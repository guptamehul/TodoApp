...
/**
* Checks whether the user is currently logged in. The method returns 'true' if 'authtoken' is already set. 
* @memberof User
...
* 1) When a session is persisted using <b>cookie(browser side cookie)</b>, authtoken would be extracted from it and a call to Built.io Backend would be made to fetch the whole user profile.</br>
...
module.exports.updateUserProfile = R.curry(function(userInfo, user) {
		...
		return adaptor.makeCall(requestObject).then(function(response) {
				var tmpUser   = user.cls.ObjectCons(module.exports.getHeaders(user),{}, R.mixin({authtoken:user.app.getAuthToken()},response.entity.application_user),{}); 
				tmpUser = tmpUser.set('authtoken', user.app.getAuthToken()); 
